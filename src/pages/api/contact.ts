...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const htmlToUser = `
    <div style="max-width:600px;margin:0 auto;padding:30px 20px;background-color:#fff;font-family:'Helvetica Neue',sans-serif;color:#222;border-radius:16px;box-shadow:0 8px 24px rgba(0,0,0,0.08);border:1px solid #eee;">
      <div style="text-align:center;margin-bottom:32px;">
        ...
        <p style="font-size:14px;color:#444;">Votre demande a bien été envoyée. Vous recevrez une réponse de ma part dans les meilleurs délais. A bientôt 👋</p>
      </div>
      <div style="font-size:15px;line-height:1.6">
        <p><strong>Nom :</strong> ${nom}</p>
        <p><strong>Prénom :</strong> ${prenom}</p>
      ...
      <div style="text-align:center;font-size:13px;color:#888;margin-top:40px;padding-top:20px;border-top:1px solid #eee">
        <p>Mail généré automatiquement par <strong>margaux-jacquet.com</strong> suite à votre demande de contact.</p>
      </div>
    </div>
  `;
  ...
  try {
    ...
    await transactionalEmailApi.sendTransacEmail({
      ...
      to: [{ email }],
      subject: '📩 Merci pour votre message, je vous réponds au plus vite ! – Margaux Jacquet',
